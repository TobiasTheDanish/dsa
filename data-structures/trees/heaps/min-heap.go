...
var _ Heap[float32] = (*MinHeap[float32])(nil)
...
func (h *MinHeap[T]) Insert(e T, prio int) error {
	node := heapNode[T]{
		prio: prio,
		data: e,
	}
...
func (h *MinHeap[T]) Find() (T, error) {
	return h.elements[0].data, nil
}

func (h *MinHeap[T]) Extract() (res T, err error) {
	if len(h.elements) == 0 {
...
func (h *MinHeap[T]) Replace(data T, prio int) (res T, err error) {
	if len(h.elements) == 0 {
...
func (h *MinHeap[T]) Delete() (err error) {
	if len(h.elements) == 0 {
...
func (h *MinHeap[T]) siftDown(index int) {
	...
	for childIndex1 < len(h.elements) && childIndex2 < len(h.elements) {
		...
		if child1.prio < node.prio && child2.prio < node.prio {
			// choose lowest child to swap with
